    ...
    <mm-atp v-loading="ui.loading">
      ...
      <template v-slot:table>
        ...
        >
          ...
          <el-table-column show-overflow-tooltip prop="F_Lev" label="等级1">
          </el-table-column>
          <el-table-column
      ...
      let that = this;
       await this.$_request({
        ...
        if (resData.F_Detail && resData.F_Detail.length) {
          that.salaryTableHeaderList = resData.F_Detail[0].F_BFKFInfo || [];
        }
        
        that.currentTableData = that.tableData = resData.F_Detail.map(x => {
